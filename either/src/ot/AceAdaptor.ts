...
class AceAdapter {
  ...
  constructor(public socket: Socket, public editor: AceAjax.Editor) {
    this.revision = 0;
  ...
  applyDeltas = (deltas: AceAjax.Delta[]) => {
    this.isSystemChange = true;
   ...
   */
  public transform(delta1: AceAjax.Delta, delta2: AceAjax.Delta): [AceAjax.Delta, AceAjax.Delta?] {
    if (!delta1) {
      return [delta2];
    } else if (!delta2) {
      return [delta1];
   ...
   */
  calcMoveDelta(start: AceAjax.Position, end: AceAjax.Position) {
    return {
   ...
   * @returns {row: number, column: number}
   */
  calcMovedPosition(pos: AceAjax.Position, delta: { row: number, column: number }) {
    return {
   ...
   * @returns boolean
   */
  hasDuplicate(delta1: AceAjax.Delta, delta2: AceAjax.Delta) {
    return (
   ...
   * @param {*} pos 位置
   */
  isDeltaIncludePosition(delta: AceAjax.Delta, pos: AceAjax.Position) {
    return (
   ...
   * @returns
   */
  isDeltaIncludeDelta(delta1: AceAjax.Delta, delta2: AceAjax.Delta) {
    return (
      delta1.start.row <= delta2.start.row &&
      delta1.start.column <= delta2.start.column &&
   ...
   * @returns
   */
  minPosition(pos1: AceAjax.Position, pos2: AceAjax.Position) {
   ...
   * @returns
   */
  maxPosition(pos1: AceAjax.Position, pos2: AceAjax.Position) {
    if (pos1.row > pos2.row) {
      return pos1;
    } else if (pos1.row === pos2.row) {
      if (pos1.column > pos2.column) {
   ...
   * @returns
   */
  mergeDeltas(delta1: AceAjax.Delta, delta2: AceAjax.Delta): AceAjax.Delta {
    // TODO:
    return delta1;
  }
}
